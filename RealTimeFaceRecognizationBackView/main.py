    ...
    if faceCurrFrame:
        ...
        if counter != 0:
            ...
            if counter == 1:
                ...
                print(studentInfo['exit_time'])


                if studentInfo['exit_time'] == "":
                    # Update exit time, whenever we place live image over webcam
                    ref = db.reference(f'Students/{id}')
                    studentInfo['exit_time'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    ref.child('exit_time').set(studentInfo['exit_time'])
            ...
            if modeType != 3:
                ...
                if counter <= 20:
                    ...
                    # show incoming time
                    cv2.putText(imgBackground, str(studentInfo['last_attendance_time']), (962, 425),
                                cv2.FONT_HERSHEY_COMPLEX, 0.5, (100, 100, 100), 1)

                    # show outgoing time
                    cv2.putText(imgBackground, str(studentInfo['exit_time']), (962, 454),
                                cv2.FONT_HERSHEY_COMPLEX, 0.5, (100, 100, 100), 1)
